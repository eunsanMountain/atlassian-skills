...
def test_issue_get_compact(snapshot: SnapshotAssertion) -> None:
    """atls jira issue get RLM-3 --format=compact matches snapshot."""
    respx.get(f"{JIRA_URL}/rest/api/2/issue/RLM-3").mock(
        return_value=httpx.Response(200, json=_load("get-issue-rlm3.json"))
    )
    result = runner.invoke(app, ["jira", "issue", "get", "RLM-3", "--format", "compact"])
...
def test_issue_get_json(snapshot: SnapshotAssertion) -> None:
    """atls jira issue get RLM-3 --format=json matches snapshot."""
    respx.get(f"{JIRA_URL}/rest/api/2/issue/RLM-3").mock(
        return_value=httpx.Response(200, json=_load("get-issue-rlm3.json"))
    )
...
def test_issue_search_compact(snapshot: SnapshotAssertion) -> None:
    """atls jira issue search 'project=RLM' --format=compact matches snapshot."""
    respx.get(f"{JIRA_URL}/rest/api/2/search").mock(
        return_value=httpx.Response(200, json=_load("search-rlm.json"))
    )
    result = runner.invoke(app, ["--format", "compact", "jira", "issue", "search", "project=RLM"])
    assert result.exit_code == 0, result.output
    assert result.output == snapshot
