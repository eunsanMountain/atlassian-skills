...
- **Current version**: 0.2.8 (keyring + shell-command credential storage with per-product commands, `atls setup` storage selection, `atls auth status --resolve`; legacy `setup all/codex/claude/paths/status` deprecated for removal in 0.3.0)
